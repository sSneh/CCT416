...
df = pd.read_csv('inputs/Cleaned_Labeled.csv')
...
with open('text_outputs/classification_report_svm.txt', 'w') as f:
    f.write(classification_report(y_test, y_pred))

# Save the classification report to a text file
# with open('classification_reportsvm.txt', 'w') as f:
#     f.write(classification_report(y_test, y_pred))
...
df_test.to_csv('text_outputs/svm.csv', index=False)
...
new_df = pd.read_csv('inputs/SG3B.csv')
...
new_df.to_csv('text_outputs/SVM_predicted.csv', index=False)
